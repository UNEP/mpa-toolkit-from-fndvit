...
async function purgeKeyValue(key: string): Promise<DistributiveOmit<EventOutput, 'event'>> {
  console.log('purging keyvalue key 👉', key);
  const response = await purgeSurrogate(`key-value-${key}`);
